...
export default function CreateDaPaintScreen() {
  ...
  useEffect(() => {
    ...
    if (typeof globalAny !== 'undefined' && globalAny.setTabBarVisibility) {
      const shouldShowTabBar = !showAd; // Show tab bar in all other cases (active DaPaint, create form, loading)
  ...
  const handleLeaveDaPaint = async () => {
    ...
    if (Platform.OS === 'web') {
      if (
        activeDaPaint.dapaint_type === '1v1' &&
        isWithin48Hours &&
        activeDaPaint.foe_id
      ) {
      ...
      if (
        ...
        Alert.alert('Leave DaPaint?', 'Are you sure you want to leave?', [
          { text: 'Cancel', style: 'cancel' },
          {
            text: 'Leave',
            style: 'destructive',
  ...
  const setPauseVideoFunction = useCallback(
    (pauseFunction: (() => void) | null) => {
    ...
    if (showAd) {
      return (
        <AdScreen
          ...
          setPauseFunction={setPauseVideoFunction}
        />
      );
    }
        ...
        onCreated={async () => {
          if (!isLoadingRef.current) {
            try {
              ...
              logger.error(
                'Error reloading data after creating DaPaint:',
                error
              );
            }
          }
